...
def main():
    for i in range(1):
        ...
        for epoch in range(epochs):
            ...
            with torch.no_grad():
                ...
                val_loss=loss_validate / val_size
        ...
        # 保存最好的参数
        torch.save(best_model, f'./data5/best_model_cnn_transformer_loss{round(best_loss, 8)}.pt')
    ...
    0: "#0", 1: "#1 ", 2: "#2", 3: "#3", 4: "#4",
        ...
        plt.savefig(f'./data5/gif_{round(best_accuracy, 6)}3.svg', dpi=600)
        plt.show()
        plt.show()
        ...
        ax1.plot(range(epochs), validate_acc, color='#038355', label='validate_acc',linewidth=2)
        ax1.plot(range(epochs), train_acc, color='#ffc34e', label='train_acc',linewidth=2)
        plt.legend()
        ...
        plt.savefig(f'./data5/gif_{round(best_accuracy, 8)}1.svg',dpi=600)
        plt.show()  # 显示 lable
        ...
        ax2.plot(range(epochs), validate_loss, color='#ae5e52', label='validate_loss',linewidth=2)
        ax2.plot(range(epochs), train_loss, color='#6b9ac8', label='train_loss',linewidth=2)
        plt.legend()
        plt.xticks(fontsize=12)
        plt.yticks(fontsize=12)
        plt.axis('on')
        ...
        plt.savefig(f'./data5/gif_{round(best_accuracy, 8)}2.svg',dpi=600)
        plt.show()  # 显示 lable
    ...
    # 保存到CSV文件
        results_df.to_csv(f'./substrate5/data5{i}.csv', index=False)
    ...
    # 假设你要读取的列名是"Column_Name"
    column_name = "Validation Accuracy"

    # 文件路径列表
    files = ["./substrate5/data50.csv", "./substrate5/data51.csv", "./substrate5/data52.csv", "./substrate5/data53.csv", "./substrate5/data54.csv"]

    # 读取每个文件的指定列并存入列表
    data_frames = [pd.read_csv(file, usecols=[column_name]) for file in files]

    # 将所有DataFrame合并为一个
    combined_df = pd.concat(data_frames, axis=1)

    # 如果你想要更改列名以区分不同文件的数据，可以这样做
    ...
    # 计算每行的标准差（除去第一行索引）
    combined_df['std_dev'] = combined_df.iloc[1:].std(axis=1)

    # 计算标准差的均值
    std_dev_mean = combined_df['std_dev'].mean()
    print("所有标准差的均值是:", std_dev_mean)

    # 计算每列的最大
    mean=[]
    for i in range(5):
        column_means = combined_df.iloc[1:, i:i+1].max()
        mean.append(float(column_means.values[0]))
    # 找到最大值的均值
    print(mean)
    max_mean = (mean[0]+mean[1]+mean[2]+mean[3]+mean[4])/5

    # 输出结果

    print("五个最大值的均值是:", max_mean)
    print("最高准确率",combined_df.max().max())
    # 保存结果到新的Excel文件
