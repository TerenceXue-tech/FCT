...
def dataloader(batch_size, workers=2):
    ...
    # test_ylabel = load('./data5/testY_1024_10c')
...
def main():
    ...
    model_path = r'C:\Users\25364\Downloads\FCT\FCT\model3\best_model_cnn_transformer_1.0.pt'
    model = torch.load(model_path)
    # 定义损失函数和优化函数
    model = model.to(device)

    learn_rate = 1e-5#调整学习率
    optimizer = torch.optim.Adam(model.parameters(), learn_rate)  # 优化器#防止过你和可以添加weight_decay=0.1
    ...
    # 保存最好的参数
    torch.save(best_model, f'./model4/best_model_cnn_transformer_{round(best_accuracy, 8)}.pt')
    ...
    plt.savefig(f'./fig4/gif_{round(best_accuracy, 6)}3.svg', dpi=600)
    plt.show()
    plt.show()
    ...
    plt.savefig(f'./fig4/gif_{round(best_accuracy, 8)}1.svg',dpi=600)
    plt.show()  # 显示 lable
    ...
    plt.savefig(f'./fig4/gif_{round(best_accuracy, 8)}2.svg',dpi=600)
    plt.show()  # 显示 lable
